...
// How long something has been waiting, floored at zero so a timestamp written
...
// rescanning the run list for each candidate, which would be quadratic in the
// number of runs the daemon is holding.
function supersededRunIds(runs: Iterable<RunMeta>): ReadonlySet<string> {
  const ids = new Set<string>();
  for (const run of runs) {
    if (run.resumedFrom !== undefined) ids.add(run.resumedFrom);
  }
  return ids;
}
...
// state no automatic path can repair (see RunMeta.baseDiscarded) — but the two
// guards below outrank even that, because they mean this run is not the thing
// a human should be looking at in the first place.
function stalledReason(
  run: RunMeta,
  superseded: ReadonlySet<string>
): StalledRunReason | null {
  if (run.archivedAt !== undefined || run.reviewedAt !== undefined) return null;
  // A resume picked this run's work up, so the successor is the live run and
  // this one is history. Mirrors the same guard in
  // Orchestrator.surveyAndUpgradeIfDirty, which refuses to upgrade a run that
  // has been taken over.
  if (superseded.has(run.id)) return null;
  // A review run that dead-ends is already represented by its task's capped
  // fix-loop item, so reporting both would show one situation as two things
  // awaiting a human.
  if (runKind(run) === 'review') return null;
...
export class DecisionFeed {
  ...
  private recompute(): {
    ...
    for (const [id, previous] of this.lastOpen) {
      ...
      // Delete first: Map.set keeps an existing key's original position, and
      // this queue is pruned oldest-first, so a re-resolved id must move to the
      // back rather than inherit where it sat the last time round.
      this.resolved.delete(id);
      this.resolved.set(id, {
        ...previous,
        ageMs: ageSince(previous.since, nowMs),
        state: 'resolved',
        resolvedAt: nowIso,
      });
    }
  ...
  private pruneResolved(nowMs: number): void {
    ...
    // Insertion order is resolution order, so walking from the front drops the
    // oldest first. The size check leads each step, which is what stops this
    // from emptying the map.
    for (const id of this.resolved.keys()) {
      if (this.resolved.size <= MAX_RESOLVED) break;
      this.resolved.delete(id);
    }
  }
  ...
  private stalledRunItems(
    ...
    const superseded = supersededRunIds(runs.values());
